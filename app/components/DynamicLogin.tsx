...
// actions work unchanged. Every phase gets visible feedback: SDK boot,
// modal, and the post-auth embedded-wallet provisioning gap.
export default function DynamicLogin() {
  const { primaryWallet, user, setShowAuthFlow, handleLogOut, sdkHasLoaded } = useDynamicContext();
  ...
  // Authenticated but the embedded wallet is still provisioning — previously
  // this window showed nothing at all.
  if (user && !primaryWallet) {
    return (
      <div className="flex items-center gap-2 rounded-xl bg-blue-900 px-6 py-3 font-semibold text-white">
        <Spinner /> Creating your wallet…
      </div>
    );
  }

  const booting = !sdkHasLoaded && !opening;
  return (
    <div>
      <button
        onClick={() => {
